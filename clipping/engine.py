...
def _cuda_available() -> bool:
    """Best-effort CUDA availability check that never raises."""
    try:
        import torch
...
def _cpu_compute_type(compute_type: str) -> str:
    """
    float16 (the GPU default) isn't a valid CTranslate2 CPU compute type.
    Downgrade to int8 (fast, good quality) unless the caller already asked
    for a CPU-safe type.
    """
...
def _transcribe_with_mlx(
    video_path: str, max_words_per_subtitle: int, model_size: str
) -> tuple[str, list[dict]]:
    """
    Transcribe using mlx-whisper, which runs on Apple Silicon's GPU via
    Metal (through Apple's MLX framework) — unlike faster-whisper/CTranslate2,
    which has no Metal backend and is CPU-only on Mac. Word-level timestamps
    come back in the same {word, start, end} shape as faster-whisper, so the
    rest of the pipeline (karaoke captions, candidate scoring) needs no
    changes downstream of this function.
    """
    import mlx_whisper
    from tqdm import tqdm

    repo = _MLX_WHISPER_REPOS.get(model_size, f"mlx-community/whisper-{model_size}-mlx")
    print(
        f"      ⏳ Memuat model Whisper '{model_size}' (Apple Metal via MLX: {repo})"
        " — unduhan pertama kali bisa memakan waktu...",
        flush=True,
    )

    result = mlx_whisper.transcribe(
        video_path, path_or_hf_repo=repo, word_timestamps=True
    )
...
def _resolve_whisper_runtime(device: str, compute_type: str) -> tuple[str, str]:
    """
    Resolve 'auto'/'cuda' device requests against actual CUDA availability,
    so a machine without a CUDA-enabled CTranslate2 build (e.g. a Mac, or a
    CPU-only server) falls back to CPU instead of crashing with
    ...
    if device == "auto":
        ...
        print(
            "      ⚠️ CUDA diminta tapi tidak tersedia di runtime ini — fallback ke CPU.",
            flush=True,
        )
        device = "cpu"
...
def transcribe_video(
    ...
    """
    device, compute_type = _resolve_whisper_runtime(device, compute_type)

    # On Apple Silicon, faster-whisper/CTranslate2 has no Metal backend and
    # would run on CPU regardless of `device`. mlx-whisper runs the same
    # model on the GPU via Metal (through Apple's MLX framework), so prefer
    # it whenever the resolved runtime would otherwise be CPU-on-Mac and the
    # optional `mlx-whisper` package is installed.
    if device == "cpu" and _mlx_whisper_available():
        print("[2/3] Memulai transkripsi dengan MLX-Whisper (Apple Metal GPU, Level Per-Kata)...")
        try:
            return _transcribe_with_mlx(video_path, max_words_per_subtitle, model_size)
        except Exception as e:
            print(
                f"      ⚠️ MLX-Whisper gagal ({e}). Fallback ke Faster-Whisper (CPU).",
                flush=True,
            )
    ...
    try:
        ...
        if device != "cpu" and "CUDA" in str(e):
            print(
                f"      ⚠️ CTranslate2 build tidak mendukung CUDA di runtime ini "
                f"({e}). Fallback ke CPU.",
                flush=True,
            )
            device, compute_type = "cpu", _cpu_compute_type(compute_type)
            model = WhisperModel(model_size, device=device, compute_type=compute_type)
        else:
            raise
